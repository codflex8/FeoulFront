...
const page = ({
  ...
  const DynamicMarker = ({ building }: { building: any }) => {
    ...
    const icon = L.divIcon({
      ...
      html: `<div style="background: rgba(255, 0, 0, 0.3); width: ${
        iconSize[0]
      }px; height: ${
        iconSize[1]
      }px; display: flex; align-items: center; justify-content: center; color: white; font-size: 16px; transition: all 0.3s ease transform" class=${clsx(
        building.id >= 12 && building.id <= 19
          ? "rotate-[-20deg]"
          : (building.id >= 20 && building.id <= 39) ||
            (building.id >= 20 && building.id <= 27) ||
            (building.id >= 55 && building.id <= 76) ||
            (building.id >= 90 && building.id <= 97)
          ? "rotate-[-35deg]"
          : ""
      )}>${building.id}</div>`,
    });
    ...
    return (
      <Marker
        ...
        icon={icon}
        eventHandlers={{
          click: () => push(`${pathname}/building-view/${building.id}`),
        }}
      />
    );
  };
